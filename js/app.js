...
(function () {
	const partners = document.querySelectorAll('.partners__item'),
		partnersRow = document.querySelector('.partners__row');
	(partners.length > 5) ? partnersRow.classList.add('multi-row') : partnersRow.classList.remove('multi-row');

	//PLANS - TARIF

	const yearly = document.querySelector('#Yearly'),
		inputPrice = document.querySelectorAll('.price__toggler-row label');

	inputPrice.forEach(item => {
		item.addEventListener("click", (e) => {
			const target = e.target,
				tarif = target.closest('label').querySelector('div');
			inputPrice.forEach(item => {
				item.querySelector('div').classList.remove('_active');
			});
			tarif.classList.add('_active');
			if (tarif === yearly) {
				document.querySelector('#tarifMonth').classList.remove('_show');
				document.querySelector('#tarifYear').classList.add('_show');
			} else {
				document.querySelector('#tarifMonth').classList.add('_show');
				document.querySelector('#tarifYear').classList.remove('_show');
			}
		});
	});
}());
